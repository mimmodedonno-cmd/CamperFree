...
function q(samples,radius){
 ...
 return `[out:json][timeout:20];(${blocks});out center tags;`
}
...
module.exports=async function(req,res){
 if(req.method==="GET")return res.status(200).json({ok:true,service:"CamperFree POI API",version:"1.7"});
 ...
 try{
  ...
  if(samples.length>16){const x=[];for(let i=0;i<16;i++)x.push(samples[Math.round(i*(samples.length-1)/15)]);samples=x}
  const corridor=Math.max(5,Math.min(20,Number(body.corridorKm)||12));
  const elements=await ask(q(samples,Math.round((corridor+3)*1000)));
  ...
  return res.status(200).json({ok:true,version:"1.7",pois:pois.slice(0,700)});
 }catch(e){
  console.error(e);
